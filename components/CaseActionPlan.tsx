    ...
    <section aria-labelledby="action-plan-title" className="premium-card overflow-hidden p-6 md:p-7">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <h2 id="action-plan-title" className="mt-2 text-3xl font-black tracking-tight text-app">Four clear steps. One safe route.</h2>
        ...
        {steps.map((step, index) => {
          ...
          return (
            <li key={step.id}>
              <Link
                href={href}
                className={`focus-ring block h-full rounded-3xl border p-5 ${step.primary ? "bg-[var(--surface-elevated)] shadow-[0_18px_46px_var(--glow)]" : "bg-app-surface"}`}
      ...
      <div className="mt-5 flex flex-wrap items-center gap-3 rounded-3xl border border-app bg-app-surface p-4">
        <ShieldCheck className="h-5 w-5 shrink-0 text-primary" />
        <p className="mr-auto text-sm font-semibold leading-6 text-app">ParkPal never submits an appeal for you. Confirm every detail, then use the official route on the notice.</p>
        {officialRoute ? (
          <a className="btn-primary focus-ring whitespace-nowrap px-4 py-2 text-sm" href={officialRoute} target="_blank" rel="noreferrer">
